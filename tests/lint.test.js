...
describe('Code Quality and Build Tests', () => {
  ...
  test('TypeScript type checking should pass', () => {
    try {
      ...
      throw new Error(`TypeScript type checking failed: ${error.stdout || error.stderr}`);
    }
  });
  ...
  test('No unescaped entities in JSX files', () => {
    const checkFile = (filePath) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes("'") && line.includes('className=') || line.includes('text=')) {
          const matches = line.match(/[^&]'[^;]/g);
          if (matches) {
            errors.push(`Line ${index + 1}: Unescaped apostrophe found: ${line.trim()}`);
          }
        }
      });
  ...
  test('No explicit any types in TypeScript files', () => {
    const checkFile = (filePath) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes(': any') && !line.trim().startsWith('//') && !line.trim().startsWith('*')) {
          errors.push(`Line ${index + 1}: Explicit 'any' type found: ${line.trim()}`);
        }
        if (line.includes('as any') && !line.trim().startsWith('//') && !line.trim().startsWith('*')) {
          errors.push(`Line ${index + 1}: 'as any' assertion found: ${line.trim()}`);
        }
      });
    ...
    const tsFiles = execSync('find src -name "*.ts" -o -name "*.tsx"', { encoding: 'utf8' })
      .split('\n')
      .filter(file => file.trim() !== '');
  ...
  test('All imports are used', () => {
    try {
      const result = execSync('npm run lint -- --quiet', { stdio: 'pipe', encoding: 'utf8' });
  ...
  test('Next.js Html component not imported outside _document', () => {
    const checkFile = (filePath) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes('import') && line.includes('Html') && line.includes('next/')) {
          errors.push(`Line ${index + 1}: Next.js Html imported outside _document: ${line.trim()}`);
        }
      });
...
describe('Vercel-specific Build Tests', () => {
  ...
  test('Production build with NODE_ENV=production', () => {
    try {
      ...
      throw new Error(`Production build failed: ${error.stdout || error.stderr}`);
    }
  });
