...
function NetworkNode({ node, onClick }: { node: Node; onClick: (id: number) => void }) {
...
function NetworkConnection({ connection, nodes }: { connection: Connection; nodes: Node[] }) {
...
function NeuralNetwork() {
  ...
  const handleNodeClick = useCallback((nodeId: number) => {
    setNodes(prev => prev.map(node => 
      node.id === nodeId 
        ? { ...node, active: true, activation: 1 }
        : node
    ))
    
    // Propagate activation
    setTimeout(() => {
      const propagateActivation = (currentNodeId: number, delay: number = 0) => {
        ...
        setTimeout(() => {
          setNodes(prev => prev.map(node => 
            connectedNodes.includes(node.id)
              ? { ...node, active: true, activation: Math.random() * 0.8 + 0.2 }
              : node
          ))
...
export function NeuralNetworkViz({ className }: NeuralNetworkProps) {
  return (
    <div className={cn("w-full h-full min-h-[600px] bg-gray-900", className)}>
      <Canvas
      ...
      >
        ...
        <Html position={[0, 6, 0]} center>
          <div className="text-white text-sm bg-black/50 p-2 rounded backdrop-blur">
