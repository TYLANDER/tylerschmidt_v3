...
export function SunIndicator() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      const sunrise = cached.sunTimes.sunrise.toLocaleTimeString([], {
        hour: "numeric",
        minute: "2-digit",
      })
      const sunset = cached.sunTimes.sunset.toLocaleTimeString([], {
        hour: "numeric",
        minute: "2-digit",
      })
    ...
    <AnimatePresence>
      {isVisible && sunInfo && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 20 }}
          className="fixed bottom-4 right-4 z-50 rounded-lg border border-border bg-card p-4 shadow-lg"
        >
