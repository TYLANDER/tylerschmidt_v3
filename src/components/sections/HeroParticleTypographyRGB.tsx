...
export function HeroParticleTypographyRGB() {
    ...
    const animate = () => {
      ...
      particles.forEach((particle) => {
        ...
        if (particle.distance < magneticRadius && particle.distance > 0) {
          const force =
            Math.pow(1 - particle.distance / magneticRadius, 2) *
            magneticStrength
          ...
          particle.currentColor.r =
            particle.baseColor.r +
            colorShift * 255 * Math.sin(hueShift * Math.PI * 2)
          particle.currentColor.g =
            particle.baseColor.g +
            colorShift *
              255 *
              Math.sin(hueShift * Math.PI * 2 + (Math.PI * 2) / 3)
          particle.currentColor.b =
            particle.baseColor.b +
            colorShift *
              255 *
              Math.sin(hueShift * Math.PI * 2 + (Math.PI * 4) / 3)

          // Clamp colors
          particle.currentColor.r = Math.max(
            0,
            Math.min(255, particle.currentColor.r)
          )
          particle.currentColor.g = Math.max(
            0,
            Math.min(255, particle.currentColor.g)
          )
          particle.currentColor.b = Math.max(
            0,
            Math.min(255, particle.currentColor.b)
          )
        } else {
          // Return to base color
          particle.currentColor.r +=
            (particle.baseColor.r - particle.currentColor.r) * 0.1
          particle.currentColor.g +=
            (particle.baseColor.g - particle.currentColor.g) * 0.1
          particle.currentColor.b +=
            (particle.baseColor.b - particle.currentColor.b) * 0.1
  ...
  return (
    <section className="relative h-screen overflow-hidden bg-black">
      ...
      <Container className="relative z-10 flex h-full flex-col justify-between py-20">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-center"
        >
          <p className="mb-4 text-sm text-gray-400">RGB Glitch Typography</p>

          <div className="mb-4 flex justify-center gap-2">
            {["TYLER SCHMIDT", "DESIGNER", "ENGINEER", "CREATIVE"].map(
              (text) => (
                <button
                  key={text}
        ...
        >
          <p className="text-xs text-gray-500">
            Move cursor near text for RGB chromatic aberration •{" "}
